...
- 9チャプターの全体像学習パス
- 実務の順序で学ぶ13の実践シナリオ（各3問、途中再開・順序を保った再挑戦）。不具合修正・引き継ぎ・並列作業・CI連携・Hook検査・最小権限・会話の再開と切り替え・チーム指示の不適用診断・定期タスク運用・Cloud環境の診断・App Server移行・MCP接続の復旧・worktreeの環境復旧を学習
- 解説を先に学ぶ「読んでから解く」モード
